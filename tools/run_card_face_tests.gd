...
const TEST_FILES := [
	"card_face_prototype_contract_test.gd",
	"card_information_hierarchy_test.gd",
]
...
func _run() -> void:
	...
	if failure_count == 0:
		print("CARD FACE TESTS PASSED (%d suites)" % TEST_FILES.size())
		quit(0)
	else:
		push_error("%d CARD FACE TEST FAILURE(S)" % failure_count)
		quit(1)
