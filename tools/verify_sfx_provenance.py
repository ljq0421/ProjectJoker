#!/usr/bin/env python3
"""Verify Project Joker SFX format, provenance, and reproducibility."""
...
ASSET_DIR = REPO_ROOT / "resources" / "audio" / "sfx"
MANIFEST_PATH = ASSET_DIR / "manifest.json"
PROVENANCE_PATH = ASSET_DIR / "PROVENANCE.md"
GENERATOR_PATH = REPO_ROOT / "tools" / "generate_sfx.py"
SERVICE_PATH = REPO_ROOT / "scripts" / "audio" / "sfx_service.gd"
...
def verify_provenance_document() -> None:
    ...
    if missing:
        raise VerificationError(
            "PROVENANCE.md is missing required statements: " + ", ".join(missing)
        )
...
def verify_wave(path: Path, entry: dict) -> None:
    ...
    if channels != 1:
        raise VerificationError(f"{path.name}: expected mono, got {channels} channels")
    if sample_rate != 48000:
        raise VerificationError(
            f"{path.name}: expected 48000 Hz, got {sample_rate}"
        )
    if sample_width != 2:
        raise VerificationError(
    ...
    if not 40.0 <= duration_ms <= 900.0:
        raise VerificationError(
            f"{path.name}: duration {duration_ms:.2f}ms outside 40-900ms"
        )
    ...
    if peak > MAX_PCM_PEAK:
        raise VerificationError(
            f"{path.name}: peak {peak} exceeds the -3 dBFS ceiling "
            f"{MAX_PCM_PEAK}"
        )
...
def verify_manifest_and_assets() -> dict[str, str]:
    ...
    if manifest.get("generator", {}).get("path") != "tools/generate_sfx.py":
        raise VerificationError("manifest generator path is not tools/generate_sfx.py")
    ...
    if set(by_file) != EXPECTED_FILES:
        ...
        raise VerificationError(
            f"manifest asset set mismatch; missing={missing}, extra={extra}"
        )
    ...
    if discovered_audio != EXPECTED_FILES:
        missing = sorted(EXPECTED_FILES - discovered_audio)
        undeclared = sorted(discovered_audio - EXPECTED_FILES)
        raise VerificationError(
            f"audio directory mismatch; missing={missing}, undeclared={undeclared}"
        )
    ...
    for file_name in sorted(EXPECTED_FILES):
        ...
        expected_parameters_sha = canonical_parameters_sha(parameters)
        if entry.get("parameters_sha256") != expected_parameters_sha:
...
def verify_runtime_references() -> None:
    ...
    referenced = set(
        re.findall(r"res://resources/audio/sfx/([a-z0-9_]+\.wav)", content)
    )
    if referenced != EXPECTED_FILES:
        missing = sorted(EXPECTED_FILES - referenced)
        extra = sorted(referenced - EXPECTED_FILES)
        raise VerificationError(
            f"runtime WAV references mismatch; missing={missing}, extra={extra}"
        )
...
def verify_reproduction(expected_hashes: dict[str, str]) -> None:
    ...
    output_dir = temp_root / "sfx_reproduction_check"
...
def main() -> int:
    ...
    parser.add_argument(
        "--reproduce",
        action="store_true",
        help="Regenerate into a temporary directory and compare hashes.",
    )
    ...
    try:
        ...
        return 1
    suffix = " with reproducible hashes" if args.reproduce else ""
    print(f"PASS: verified {len(hashes)} commercial-provenance SFX assets{suffix}.")
    return 0
