class_name FacelessHubGuideProgressStore
extends RefCounted

const SECTION := "faceless_hub_guide_v1"
const DISMISSED_KEY := "dismissed"
const CHECKPOINT_KEY_BY_ID := {
	&"composite": "seen_composite",
	&"schedule": "seen_schedule",
	&"restriction": "seen_restriction",
}
...
func _load_existing() -> void:
	...
	if result != OK:
		...
		_write_blocked = true
		return
	_values[DISMISSED_KEY] = bool(config.get_value(
		SECTION,
		DISMISSED_KEY,
		false
	))
